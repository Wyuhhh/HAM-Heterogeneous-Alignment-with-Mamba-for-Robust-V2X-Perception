...
def _normalize_timestamp_token(ts) -> str:
    """Normalize timestamp/frame token read from configs/logs.

    We keep it as a string to match folder names like "000123" or "123".
    """
    if ts is None:
        return ""
    ts = str(ts).strip()
    # tolerate accidental suffix like ".pkl"
    ts = ts.replace(".pkl", "")
    return ts


def _extract_frame_id_from_scenario_dict(scenario_dict, ego_type: str):
    """Try best-effort to infer the timestamp/frame id from parse_seq output.

    The exact structure of parse_seq can vary across datasets/branches.
    We only rely on it being an ordered mapping where each ego frame has a
    dictionary with keys containing the frame identifier.
    """
    if not scenario_dict:
        return None

    # Get ego cav id (first key should already be ego after reordering)
    try:
        ego_cav = next(iter(scenario_dict.keys()))
    except StopIteration:
        return None

    ego_frames = scenario_dict.get(ego_cav)
    if not isinstance(ego_frames, dict) or len(ego_frames) == 0:
        return None

    # pick the first frame key
    frame_key = next(iter(ego_frames.keys()))
    # Common cases: "000123" or path-like ".../000123" or int
    if isinstance(frame_key, (int, np.integer)):
        return str(int(frame_key))
    if isinstance(frame_key, str):
        # if it's a path, take basename
        return os.path.basename(frame_key.rstrip("/"))
...
class BaseDataset(Dataset):
    ...
    def __init__(self, params, visualize, train=True):
        ...
        # Optional dataset blacklist: filter problematic scenarios by folder name.
        # Example: ["2025_05_06_11_03_25", "2025_05_06_07_58_11"]
        ignore_scenarios = set(
            params.get("data_filter", {})
            .get("train" if self.train else "val", {})
            .get("ignore_scenarios", [])
            or []
        )

        # Optional fine-grained blacklist: filter problematic frames by (scenario, timestamp).
        # Example:
        #   ignore_frames:
        #     - {scenario: "2025_05_06_11_03_25", timestamp: "395"}
        #     - {scenario: "2025_05_06_11_03_25", timestamp: 397}
        ignore_frames_cfg = (
            params.get("data_filter", {})
            .get("train" if self.train else "val", {})
            .get("ignore_frames", [])
        ...
        ignore_frames = set()
        for item in ignore_frames_cfg:
            if not isinstance(item, dict):
                continue
            sc = item.get("scenario") or item.get("scenario_name")
            ts = item.get("timestamp") or item.get("frame") or item.get("ts")
            sc = str(sc).strip() if sc is not None else ""
            ts = _normalize_timestamp_token(ts)
            if sc and ts:
                ignore_frames.add((sc, ts))

        def _filter_blacklisted_frames_in_place(sd, scenario_name: str):
            """Remove blacklisted timestamps from parse_seq output.

            This is done during dataset initialization to keep idx->sample mapping
            identical across all DDP ranks.
            """
            if not ignore_frames:
                return

            # (scenario, ts) blacklist only applies to this scenario.
            black_ts = {
                ts for (sc, ts) in ignore_frames if sc == scenario_name and ts
            }
            if not black_ts:
                return

            for cav_id, cav_frames in list(sd.items()):
                if not isinstance(cav_frames, dict):
                    continue
                # Keep meta keys like 'ego' untouched.
                for fk in list(cav_frames.keys()):
                    if fk == "ego":
                        continue
                    ts_norm = _normalize_timestamp_token(fk)
                    if ts_norm in black_ts:
                        cav_frames.pop(fk, None)
        ...
        for scenario_folder in tqdm(
            ...
            # Fine-grained filtering by (scenario, timestamp) must be done while building
            # the dataset index; otherwise DDP ranks may see different sample streams.
            _filter_blacklisted_frames_in_place(scenario_dict, scenario_name)
            ...
            while True:
                ...
                if next(iter(self.scenario_database[i][first].items()))[1][
                    ...
                    # In case filtering removed all frames for this scenario, skip it.
                    if record_length == 0:
                        # remove this scenario entry we just added
                        self.scenario_database.pop(i, None)
                        break
        ...
        if len(self.len_record) == 0:
            raise RuntimeError(
                f"No samples found after applying data_filter.ignore_scenarios="
                f"{sorted(list(ignore_scenarios))}. Please check dataset root_dir/validate_dir "
                f"and the blacklist tokens."
            )

        # persist for per-index filtering
        self.ignore_scenarios = ignore_scenarios
