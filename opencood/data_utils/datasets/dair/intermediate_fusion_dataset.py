...
class IntermediateFusionDatasetDair(basedataset.BaseDatasetDair):
    """
    ...
    def __init__(self, params, visualize, train=True):
        super(IntermediateFusionDatasetDair, self).__init__(params, visualize, train)
        ...
        filter_cfg = params.get("data_filter", {})
        self.drop_empty_lidar_agents = filter_cfg.get("drop_empty_lidar_agents", True)
        self.keep_ego_when_empty = filter_cfg.get("keep_ego_when_empty", True)
        self.min_lidar_points = max(0, filter_cfg.get("min_lidar_points", 1))
        self.empty_lidar_log_cap = filter_cfg.get("empty_lidar_log_cap", 20)
        # When keeping empty lidar agents (min_lidar_points=0), suppress the noisy
        # dummy-point warning from sp_voxel_preprocessor to avoid log spam.
        self.suppress_empty_pointcloud_warning = filter_cfg.get(
            "suppress_empty_pointcloud_warning", True
        )
        if self.suppress_empty_pointcloud_warning:
            warnings.filterwarnings("ignore", category=EmptyPointCloudWarning)
        self._empty_lidar_logged = 0
    ...
    def __getitem__(self, idx):
        ...
        # Handle empty object_stack case
        if object_stack.shape[0] > 0 and len(unique_indices) > 0:
        ...
        all_class_id_stack = np.array(all_class_id_stack) if len(all_class_id_stack) > 0 else np.array([], dtype=np.int64)
        all_class_id_stack = all_class_id_stack[unique_indices] if len(unique_indices) > 0 else np.array([], dtype=np.int64)
    ...
    def get_item_single_car(self, selected_cav_base, ego_pose):
        ...
        is_ego = bool(selected_cav_base.get("ego", False))
        ...
        # Note: Handle empty / low-point lidar. Drop agents with insufficient points when configured.
        point_count = lidar_np.shape[0]
        if point_count < self.min_lidar_points:
            should_drop = self.drop_empty_lidar_agents and not (
                is_ego and self.keep_ego_when_empty
            )
            if should_drop:
                if self._empty_lidar_logged < self.empty_lidar_log_cap:
                    cav_id = selected_cav_base.get("cav_id", "?")
                    warnings.warn(
                        f"Dropped {agent_type} cav={cav_id} because lidar only had {point_count} points.",
                        EmptyPointCloudWarning,
                    )
                    self._empty_lidar_logged += 1
                return None
            if point_count == 0:
