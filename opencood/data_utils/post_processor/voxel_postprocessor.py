...
class VoxelPostprocessor(BasePostprocessor):
    ...
    def post_process_airv2x(self, data_dict, output_dict):
        ...
        for cav_id, cav_content in data_dict.items():
            ...
            B, AC, H, W = psm.shape
            C = self.num_class  # fixed: getattr on dict does not work
            A = AC // C

            # FIX: reshape must match loss: permute then view, not view(C,A) then permute
            psm = psm.permute(0, 2, 3, 1).contiguous()  # [B, H, W, A*C]
            psm = psm.view(B, H, W, A, C)  # [B, H, W, A, C]
            prob = torch.sigmoid(psm)
            prob = prob.view(1, -1, C)  # [1, H*W*A, C]]
            # prob = prob[:, :, 1:]  # FIXED: Do not remove class 0 (car)
            
            class_scores, class_labels = torch.max(prob, dim=-1)  # [1, N] 
            # class_labels = class_labels + 1  # FIXED: Labels already start from 0
