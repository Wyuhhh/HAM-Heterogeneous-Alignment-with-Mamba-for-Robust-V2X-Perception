...
class SpVoxelPreprocessor(BasePreprocessor):
    ...
    def _format_context_suffix(self):
        ...
        items = []
        for key, label in interested_keys:
    ...
    def preprocess(self, pcd_np):
        ...
        if empty_flag:
            ...
            with warnings.catch_warnings():
                warnings.warn("Warning: empty point cloud. Add dummy points. Add dummy points. "
                              ...
                              "It will be solved in the future dataset version." + self._format_context_suffix(), 
                              EmptyPointCloudWarning)
            if self.train and self.empty_point_frames % 50 == 0:
                print(
                    f"[SpVoxelPreprocessor] Empty point frames: {self.empty_point_frames}/{self.total_frames}"
                )
        ...
        data_dict["empty_point_cloud"] = empty_flag
        if self.total_frames > 0:
            data_dict["empty_point_ratio"] = self.empty_point_frames / self.total_frames

        # reset context after one use to avoid leaking stale metadata
        self.context = {}
