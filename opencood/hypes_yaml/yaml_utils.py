...
try:
    # Centralized dataset path fallbacks
    from opencood.data_utils import dataset_paths as _ds_paths
except Exception:
    _ds_paths = None


def _override_dataset_dirs_from_env(param):
    """
    Override dataset directories using environment variables if provided.
    Supported env vars:
      - AIRV2X_DATA_ROOT / OPENCOOD_DATA_ROOT / DATASET_ROOT
      - AIRV2X_TRAIN_DIR / OPENCOOD_TRAIN_DIR
      - AIRV2X_VAL_DIR   / OPENCOOD_VAL_DIR
      - AIRV2X_TEST_DIR  / OPENCOOD_TEST_DIR
    """
    root = (
        os.environ.get("AIRV2X_DATA_ROOT")
        or os.environ.get("OPENCOOD_DATA_ROOT")
        or os.environ.get("DATASET_ROOT")
    )
    train_dir = os.environ.get("AIRV2X_TRAIN_DIR") or os.environ.get(
        "OPENCOOD_TRAIN_DIR"
    )
    val_dir = os.environ.get("AIRV2X_VAL_DIR") or os.environ.get("OPENCOOD_VAL_DIR")
    test_dir = os.environ.get("AIRV2X_TEST_DIR") or os.environ.get(
        "OPENCOOD_TEST_DIR"
    )

    if isinstance(param, dict):
        # top-level dataset dirs
        if "root_dir" in param:
            if train_dir:
                param["root_dir"] = train_dir
            elif root:
                param["root_dir"] = os.path.join(root, "train")
        if "validate_dir" in param:
            if val_dir:
                param["validate_dir"] = val_dir
            elif root:
                param["validate_dir"] = os.path.join(root, "val")
        if "test_dir" in param:
            if test_dir:
                param["test_dir"] = test_dir
            elif root:
                param["test_dir"] = os.path.join(root, "test")

        # Fallback to centralized paths if provided paths do not exist
        def _need_fallback(p):
            return (p is None) or (isinstance(p, str) and not os.path.isdir(p))

        if _ds_paths is not None:
            rd = param.get("root_dir")
            vd = param.get("validate_dir")
            td = param.get("test_dir")
            if _need_fallback(rd) and hasattr(_ds_paths, "TRAIN_DIR"):
                param["root_dir"] = _ds_paths.TRAIN_DIR
            if _need_fallback(vd) and hasattr(_ds_paths, "VAL_DIR"):
                param["validate_dir"] = _ds_paths.VAL_DIR
            if _need_fallback(td) and hasattr(_ds_paths, "TEST_DIR"):
                param["test_dir"] = _ds_paths.TEST_DIR
...
def load_yaml(file, opt=None):
    ...
    # Keep control-flow explicit so type checkers/language servers
    # don't warn about unbound locals in edge paths.
    stream = open(file, "r")
    loader = yaml.Loader
    ...
    param = yaml.load(stream, Loader=loader) or {}
    if "yaml_parser" in param:
        param = eval(param["yaml_parser"])(param)

    # apply dataset directory overrides from environment variables if provided
    # NOTE: keep a single return path so static analyzers don't think `param`
    # might be unbound.
    param = _override_dataset_dirs_from_env(param)
    return param or {}
...
def load_pickle(file):
    ...
    try:
        ...
        traceback.print_exc()
        # Historically this dropped into pdb; in non-interactive runs that can
        # hang. Return an empty dict so callers can surface a clearer error.
        return {}
...
def load_airv2x_params(param):
    ...
    param["model"]["args"]["drone"]["lidar"]["point_pillar_scatter"]["grid_size"] = drone_grid_size
    ...
    # Ensure fusion_backbone config is placed correctly.
    # Some YAMLs store it under model->args, but some training code expects it under model->args['fusion_backbone'].
    # If the YAML nests fusion_backbone under model->args['modality_fusion'], lift it up.
    model_args = param.get("model", {}).get("args", {})
    modality_fusion = model_args.get("modality_fusion", {}) if isinstance(model_args, dict) else {}
    if isinstance(modality_fusion, dict) and "fusion_backbone" in modality_fusion and "fusion_backbone" not in model_args:
        model_args["fusion_backbone"] = modality_fusion["fusion_backbone"]
        param["model"]["args"] = model_args
...
def load_airv2x_bm2cp_params(param):
    ...
    rsu_lidar_range = param["model"]["args"]["rsu"]["pc_params"]["lidar_range"]
    rsu_grid_size = (
        np.array(rsu_lidar_range[3:6]) - np.array(rsu_lidar_range[0:3])
    ) / np.array(voxel_size)
    rsu_grid_size = np.round(rsu_grid_size).astype(np.int64)
