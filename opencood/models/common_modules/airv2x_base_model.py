...
class Airv2xBase(nn.Module):
    ...
    def extract_features(self, data_dict):
        ...
        # Use ego record_len to determine the true batch size
        if "ego" in data_dict and "record_len" in data_dict["ego"]:
             B = data_dict["ego"]["record_len"].shape[0]
        else:
             # Fallback to original logic if ego is missing (unlikely)
             B = max(len(data_dict["vehicle"]["batch_idxs"]), 
                     len(data_dict["rsu"]["batch_idxs"]),
                     len(data_dict["drone"]["batch_idxs"]))
