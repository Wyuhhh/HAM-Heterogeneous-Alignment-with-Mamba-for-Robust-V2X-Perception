...
class MultiModalFusion(nn.Module):
    def __init__(self, feature_dims):
        ...
        self.lidar_fusion = MaxFusion()
        self.camera_fusion = AttFusion(feature_dims)

    def forward(self, lidar_features, camera_features, record_len, affine_matrix):
        """
        Multi-modal fusion forwarding.

        Parameters
        ----------
        lidar_features : torch.Tensor
            LiDAR input data, (sum(n_cav), C, H, W)

        camera_features : torch.Tensor
            Camera input data, (sum(n_cav), C, H, W)
        ...
        """
        # Fuse LiDAR features
        fused_lidar = self.lidar_fusion(lidar_features, record_len, affine_matrix)

        # Fuse Camera features
        fused_camera = self.camera_fusion(camera_features, record_len, affine_matrix)

        # Combine LiDAR and Camera features
        fused_features = fused_lidar + fused_camera
