...
class CamEncode_Resnet101(nn.Module):  # 提取图像特征进行图像编码
    ...
    def get_gt_depth_dist(self, x):  # 对深度维进行onehot，得到每个像素不同深度的概率
        ...
        depth_indices = torch.nan_to_num(depth_indices, nan=0.0, posinf=float(self.num_bins - 1), neginf=0.0)
        depth_indices = torch.clamp(depth_indices, min=0, max=self.num_bins - 1)
