...
class BasicBlock(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        out = self.conv1(x)
        # Ensure BN runs with matching dtype (AMP may produce Half inputs)
        out = self.bn1(out.to(self.bn1.weight.dtype))
        out = self.relu(out)

        out = self.conv2(out)
        out = self.bn2(out.to(self.bn2.weight.dtype))
...
class Bottleneck(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        out = self.conv1(x)
        # Avoid explicit dtype casts that can create extra large temporary tensors
        # under AMP (and therefore inflate peak memory).
        out = self.bn1(out)
        out = self.relu(out)
