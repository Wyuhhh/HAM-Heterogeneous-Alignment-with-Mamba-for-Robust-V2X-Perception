...
def test_parser():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="max number of samples to infer, 0 means all",
    )
    parser.add_argument(
        "--save_npy",
        action="store_true",
...
def main():
    ...
    if "test_dir" in hypes and (not os.path.exists(hypes["test_dir"])):
        fallback_test_dir = "/data1/wangyh/airdata/AirV2X-Perception/val/val"
        if os.path.exists(fallback_test_dir):
            hypes["test_dir"] = fallback_test_dir
            hypes["validate_dir"] = fallback_test_dir
    ...
    for i, batch_data in tqdm(enumerate(data_loader)):
        ...
        with torch.no_grad():
            ...
            if opt.fusion_method == "late":
                ...
                import json
                try:
                    with open(r'd:\daima\1.14\heal_framework\.cursor\debug.log', 'a') as f:
                        f.write(json.dumps({"sessionId":"debug-session","runId":"pre-fix","hypothesisId":"B","location":"inference_airv2x.py:184","message":"Before inference_intermediate_fusion_airv2x call","data":{"batch_idx":i},"timestamp":int(__import__('time').time()*1000)}) + '\n')
                except: pass
                # #endregion

                pred_box_tensor, pred_score, pred_labels, pred_boxes3d, gt_box_tensor, gt_class_label_list, gt_track_list = (
                ...
                try:
                    with open(r'd:\daima\1.14\heal_framework\.cursor\debug.log', 'a') as f:
                        f.write(json.dumps({"sessionId":"debug-session","runId":"pre-fix","hypothesisId":"B","location":"inference_airv2x.py:192","message":"After inference_intermediate_fusion_airv2x unpack","data":{"pred_boxes3d_shape":str(pred_boxes3d.shape) if hasattr(pred_boxes3d,'shape') else "None","unpacked_count":7},"timestamp":int(__import__('time').time()*1000)}) + '\n')
                except: pass
                # #endregion
            ...
            if opt.save_pred:
                ...
                inference_utils.save_preds_airv2x(pred_box_tensor, pred_score, pred_boxes3d, batch_data, saved_path, pred_labels)
