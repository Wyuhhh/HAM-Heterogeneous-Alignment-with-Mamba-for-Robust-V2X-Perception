...
def load_saved_model(saved_path, model, epoch=None):
    ...
    #    return 100, model
        ...
        checkpoint_path = os.path.join(saved_path, "net_epoch%d.pth" % initial_epoch)
        if initial_epoch == 0 and not os.path.exists(checkpoint_path):
            print(f"No checkpoint found at {checkpoint_path}, starting from scratch.")
            return 0, model

        map_location = "cuda:0" if torch.cuda.is_available() else "cpu"
        state_dict_ = torch.load(checkpoint_path, map_location=map_location)
        state_dict = {}
...
def load_model(saved_path, model, epoch=None, start_from_best=True):
    ...
    if initial_epoch > 0:
        ...
        map_location = "cuda:0" if torch.cuda.is_available() else "cpu"
        state_dict_ = torch.load(model_path, map_location=map_location)
        state_dict = {}
        
        # Extract model_state_dict if checkpoint is a dict with metadata
        if isinstance(state_dict_, dict) and 'model_state_dict' in state_dict_:
            raw_state_dict = state_dict_['model_state_dict']
        else:
            raw_state_dict = state_dict_
        
        # Convert data_parallel to model (remove "module." prefix from DDP)
        for k in raw_state_dict:
            if k.startswith("module") and not k.startswith("module_list"):
                state_dict[k[7:]] = raw_state_dict[k]
            else:
                if k.startswith("cdd"):
                    # rename cdd to mdd
                    state_dict["m" + k[1:]] = raw_state_dict[k]
                else:
                    state_dict[k] = raw_state_dict[k]
