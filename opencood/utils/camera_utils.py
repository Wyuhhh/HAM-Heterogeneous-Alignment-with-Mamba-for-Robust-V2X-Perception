...
def ue4_to_lss(camera_to_lidar):
    """
    Convert/normalize extrinsic from UE4 to LSS expected convention.
    Placeholder: returns input unchanged.
    Args:
        camera_to_lidar (np.ndarray): 4x4 transform from camera to lidar/world.
    Returns:
        np.ndarray: transformed extrinsic (currently passthrough).
    """
    return camera_to_lidar
...
def draw_bin_mask(heatmap, gt_box2d, gt_box2d_mask):
    for gt_box in gt_box2d[gt_box2d_mask]:
        ...
        # 检查 convex_hull 是否为多边形
        print(f"convex_hull type: {type(poly)}")
        if poly.geom_type == 'Polygon':
            try:
                exterior_coords = list(poly.exterior.coords)
                cv2.fillConvexPoly(
                    heatmap, np.array(exterior_coords, dtype=np.int32), 1
                )
            except AttributeError:
                print("Error: Unable to access 'exterior' attribute of the polygon.")
    return heatmap
...
def coord_3d_to_2d(
    ...
    #     plt.savefig(f"/dssg/home/acct-eezy/eezy-user1/yifanlu/OpenCOOD-main/vis_result/2d_gt_boxes/fg_mask_{idx}.png", dpi=300)
