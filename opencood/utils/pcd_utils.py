...
_COORD_CORRECTION_NOTICE_EMITTED = False
_COORD_CORRECTION_SENTINEL = os.environ.get(
    "OPENCOOD_COORD_NOTICE_SENTINEL", "/tmp/opencood_coord_notice_once"
)


def _log_coordinate_correction_once():
    """Log the correction notice only once across processes.

    Uses an in-memory flag plus a filesystem sentinel so that DDP workers and
    dataloader worker processes do not each emit their own copy.
    """
    global _COORD_CORRECTION_NOTICE_EMITTED
    if _COORD_CORRECTION_NOTICE_EMITTED:
        return

    # Try to create a sentinel file atomically; if it already exists, skip.
    try:
        fd = os.open(_COORD_CORRECTION_SENTINEL, os.O_CREAT | os.O_EXCL | os.O_WRONLY)
        os.close(fd)
        should_log = True
    ...
    if should_log:
        print(
            "Applying coordinate system correction (legacy AirV2X LiDAR detected). "
            "Further messages suppressed."
        )
    _COORD_CORRECTION_NOTICE_EMITTED = True
...
def pcd_to_np(pcd_file, get_weather=False, correct_coordinate_system=False):
    ...
    if correct_coordinate_system:
        _log_coordinate_correction_once()
